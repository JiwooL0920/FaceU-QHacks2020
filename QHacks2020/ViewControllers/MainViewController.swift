...
//        let model = Resnet50()
        ...
        let model2 = GenderNet()
        let model3 = AgeNet()
//        let model4 = Inceptionv3()
        let model4 = Resnet50()
    
        @IBAction func analyzeButtonTapped(_ sender: Any) {
            guard let result = analyzeButtonAction() else { return }
            emotion = result
//            print(emotion)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender:Any?) {
        ...
        vc.emotionResult = self.emotion
        vc.genderResult = self.gender
        vc.ageResult = self.age
    }
    
        //------------------------------------------------------------------------------------------
        //Result post
    func resultPost(result:String) {
        guard let currentUser = Auth.auth().currentUser else { return }
        print(currentUser.uid)
        db.collection("users").document(currentUser.uid).getDocument { (snapshot, error) in
//            print("hello world")
//            print(snapshot?.data()!) //nil["firstname" : firstName, "lastname" : lastName, "uid" : result!.user.uid, "predictions":[] ])
            if var predictions = snapshot?.data()?["predictions"] as? [[String: Any]] {
                print(predictions) //nil
                // Set new pre
//                predictions.append(result)
                self.db.collection("users").document(currentUser.uid).updateData([ "predictions": FieldValue.arrayUnion([
                    ["id": UUID().uuidString, "emotion": result]
                ]) ])
//                let resultresult = snapshot?.data()?["predictions"] as? [[String: Any]]
//                print(resultresult)
            }
        }
    }
...
//    self.view.window?.makeKeyAndVisible()
    func changeView() {
        let selectionViewController = self.storyboard?.instantiateViewController(identifier: Constants.Storyboard.selectionViewController) as? SelectionViewController
        self.view.window?.rootViewController = selectionViewController
        self.view.window?.makeKeyAndVisible()
    }
        //----------------------------------------------------------
...
//            TODO: Figure out orientation
            ...
            if vc.sourceType == .camera {
                performVisionRequest(image: cgImage,
                                     orientation: .right)//.right for take photo, .up for upload photo
            } else if vc.sourceType == .photoLibrary {
                performVisionRequest(image: cgImage,
                                     orientation: .up)//.right for take photo, .up for upload photo
            } else if vc.sourceType == .savedPhotosAlbum {
                performVisionRequest(image: cgImage,
                                     orientation: .right)//.right for take photo, .up for upload photo
            }
        ...
        fileprivate func handleDetectedFaceLandmarks(request: VNRequest?, error: Error?) {
            ...
            DispatchQueue.main.async {
                ...
                //Image add logo on right eye <<---
//                for result in results {
//                    guard let landmark = result.landmarks else { continue }
//                    print("hello")
//                    guard let rightEye = landmark.rightEye else { continue }
//                    print("Hello world")
//                    let icon = UIImage(named: "icon")!
//                    let iconView = UIImageView(image: icon)
//                    let size = CGSize(width: self.imageWidth, height: self.imageHeight)
//                    let point = rightEye.pointsInImage(imageSize: size).first!
//                    print(point)
//                    iconView.frame = CGRect(x: 0, y: 0, width: 150, height: 150)
...
//                }
    ...
    //What to do when we press analyzee
    func analyzeButtonAction() -> String? {
        guard let imagee = imageView.image?.cgImage else { return nil }
        //Emotion
        guard let resizedImage = resizeImage(image: UIImage(cgImage: imagee), targetSize: CGSize(width: 224.0, height: 224.0)).cgImage else { return nil }
        guard let image = resizedImage.cropping(to: CGRect(x: 0, y: 0, width: 224, height: 224)) else {
            return nil
        }
        guard let buffer = image.pixelBuffer(width: image.width, height: image.height, orientation: .up) else {
            print("Buffer invalid")
            return nil
        }
        
        //Age and Gender
        guard let resizedImage2 = resizeImage(image: UIImage(cgImage: imagee), targetSize: CGSize(width: 227.0, height: 227.0)).cgImage else { return nil }
        guard let image2 = resizedImage2.cropping(to: CGRect(x: 0, y: 0, width: 227.0, height: 227.0)) else {
            return nil
        }
        guard let buffer2 = image2.pixelBuffer(width: image2.width, height: image2.height, orientation: .up) else {
            print("Buffer invalid")
            return nil
        }
        
//        //try printing image on page
//        guard let testImage = UIImage(pixelBuffer: buffer) else { return nil }
//        let testImageView = UIImageView(image: testImage)
//        testImageView.frame = CGRect(x: 0, y: 0, width: 224, height: 224)
//        testImageView.center = CGPoint(x: 150, y: 150)
//        self.view.addSubview(testImageView)

//        let input = Resnet50Input(image: buffer)

        
        let input = CNNEmotionsInput(data: buffer)
        let prediction = try! model.prediction(input: input)
        print("Prediction:")
        print(prediction.classLabel)
        emotion = prediction.classLabel
        
        let input2 = GenderNetInput(data: buffer2)
        let prediction2 = try! model2.prediction(input: input2)
        print("Prediction:")
        print(prediction2.classLabel)
        gender = prediction2.classLabel
        
        let input3 = AgeNetInput(data: buffer2)
        let prediction3 = try! model3.prediction(input: input3)
        print("Prediction:")
        print(prediction3.classLabel)
        age = prediction3.classLabel
        
        
        //299x299 -- Inceptionv3.mlmodel
//        guard let resizedImage4 = resizeImage(image: UIImage(cgImage: imagee), targetSize: CGSize(width: 299.0, height: 299.0)).cgImage else { return nil }
//        guard let image4 = resizedImage4.cropping(to: CGRect(x: 0, y: 0, width: 299.0, height: 299.0)) else {
//            return nil
//        }
//        guard let buffer4 = image4.pixelBuffer(width: image4.width, height: image4.height, orientation: .up) else {
//            print("Buffer invalid")
//            return nil
//        }
//        guard let prediction4 = try? model4.prediction(image: buffer4) else {
//            return nil
//        }
//        print("Prediction4!!!!")
//        print(prediction4.classLabel)
        
        let input4 = Resnet50Input(image: buffer)
        let prediction4 = try! model4.prediction(input: input4)
        print("REZNET IMAGE RECOGNITION PREDICTION")
        print(prediction4.classLabel)
